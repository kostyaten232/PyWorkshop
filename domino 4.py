...
class Domino:
    ...
    def get_ai_command(self):
      move_options = [opt for opt in self.computer if opt[0] in self.snake_end or opt[1] in self.snake_end]
      if move_options:
    ...
    def get_stats(self, game, turn=None):
      ...
      print('{}{}{}'.format(
                ''.join(list(map(str, self.snake[:3]))) if len(self.snake) > 6 else ''.join(list(map(str, self.snake))), '...' if len(self.snake) > 6 else '',
